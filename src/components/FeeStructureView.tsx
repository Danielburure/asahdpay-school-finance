...
export function buildFeeStructureHtml(
  ...
  const rows = classes
    .map(
      (c) => `<tr>
        ...
        <td style="padding:10px;border-bottom:1px solid #e5e7eb;text-align:right">KES ${(
          fees[c] || 0
        ).toLocaleString()}</td>
      </tr>`,
    )
  ...
  <style>
    body{font-family:ui-sans-serif,system-ui,sans-serif;color:#0f172a;max-width:780px;margin:32px auto;padding:24px;background:#fff}
    ...
    <h2>Fee Structure</h2>
    <table>
      <thead><tr><th>Class</th><th>Fees (per term)</th></tr></thead>
      <tbody>
        ${rows || `<tr><td colspan="2" style="padding:16px;text-align:center;color:#64748b">No classes added yet</td></tr>`}
      </tbody>
    </table>
